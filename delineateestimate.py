...
def execute_species_partition_estimation(tree_file, constraints_file,
output_file, extra_info_field_value,
...
version= False):
    ...
    controller = control.get_controller(tree_file, constraints_file,
    output_file)
    #for func in funcs: func()

    if speciation_completion_rate is not None:
        controller.speciation_completion_rate = speciation_completion_rate
    ...
    if controller.has_species_constraints:
        ...
        inclusion_set = [nd.taxon for nd in tree.leaf_node_iter() if nd.taxon.label in labels]
        induced_tree = dendropy.Tree(tree)
        induced_tree.retain_taxa(inclusion_set)
        #print(dir(induced_tree))
    ...
    extra_fields = utility.parse_fieldname_and_value(extra_info_field_value)
    if extra_fields:
    ...
    result_dict["report_mle_only"] = report_mle_only
    result_dict["report_constrained_probability_threshold"] = report_constrained_probability_threshold
    result_dict["report_constrained_cumulative_probability_threshold"] = report_constrained_cumulative_probability_threshold
    ...
    dest_tree_files = []
    if not no_primary_tree_file:
        treefile_path, treefile = open_output_file(constraints_file, output_file,
                suffix=".delimitation-results",
                extension="trees")
        dest_tree_files.append(treefile)
    if truncated_tree_file_size > 0:
        trunc_treefile_path, trunc_treefile = open_output_file(constraints_file, output_file,
                suffix=".delimitation-results",
    ...
    if store_relabeled_trees is not None:
        relabeled_trees_path, rltf = open_output_file(constraints_file, output_file,
            suffix=".delimitation-results.relabeled",
            extension="trees")
        if store_relabeled_trees == "species-lineage":
            relabel_key = "species-lineage"
        elif store_relabeled_trees == "lineage-species":
            relabel_key = "lineage-species"
        else:
            raise ValueError(store_relabeled_trees)
    ...
    with contextlib.ExitStack() as stack:
        ...
        if not no_translate_tree_tokens:
            _wt("        Translate\n")
        ...
        for key_idx, (key, key_as_list, prob) in enumerate(species_partition_info):
            #for func in funcs: func()
            if report_mle_only and key_idx > 0:
                break
            if key_idx == truncated_tree_file_size:
            ...
            if report_constrained_probability_threshold is not None and p["constrained_probability"] < report_constrained_probability_threshold:
                break
            ...
            if report_constrained_cumulative_probability_threshold is not None and cumulative_probability_given_constr > report_constrained_cumulative_probability_threshold:
                break
            ...
            for nd in leaf_nodes:
                ...
                if figtree_display_label != "clear":
            ...
            xx.append(p["species_leafsets"])
            print(xx)
            kk.append(p["constrained_probability"])
            print(kk)
        ...
        if figtree_display_label != "clear":
            _wt("\n\nbegin figtree;\n")
            if figtree_display_label == "species":
                _wt('set tipLabels.displayAttribute="species";\n');
            elif figtree_display_label == "species-lineage":
                _wt('set tipLabels.displayAttribute="species-lineage";\n');
            elif figtree_display_label == "lineage-species":
    ...
    #xr= [y.replace(str(x)) for x, y in zip(range(1, len(xx)+1), list(xx.keys()))]
    ...
    outf1.write(f"N_spartitions = 1 : {file}_Delineate, {pval};\n")
    outf1.write(f'N_individuals = {sum(len(x) for x in list(xy.values()))};\n')
    outf1.write(f'N_subsets={len(xy.keys())};\n')
    outf1.write("[Generated by Delineate]\n")
    outf1.write("[WARNING: The sample names below may have been changed to fit SPART specification (only alphanumeric characters and _ )]\n")
    outf1.write("Individual_assignment = \n")

    ll=[]

    io= re.compile(r"[^A-Za-z0-9]")

    for c, v in list(xy.items()):
        for t in v:
            yy= str(t)
            yy= io.sub("_", yy)
            xm= yy + ":" + repr(c)
...
the one with the\nhighest conditional probability. The SPART output is\ncurrently designed to deal only with DELINEATE\nanalyses in the typical implementation, i.e., species\naffinities for some lineages known a priori. Note that\nthe conditional probability corresponds to the likelihood of\nthe partition. Additional partitions and their\nprobabilities are included in the JSON outputfile]")
    ...
    if True: # args.write_json
        out_path, outf = open_output_file(constraints_file, output_file,
                suffix=".delimitation-results",
                extension="json")
        with outf:
...
def execute_speciation_completion_rate_estimation(tree_file= None, constraints_file= None,
output_file= None, extra_info_field_value=None,
figtree_display_label='species-lineage',
no_primary_tree_file=False,
no_translate_tree_tokens=False,
preserve_underscores=True,
...
version= False):
    ...
    out_path, out = open_output_file(constraints_file, output_file,
            suffix=".rate-results",
            extension="tsv")
    with out:
...
def open_output_file(constraints_file, output_file, suffix= None, extension= None):
    if output_file == "-":
        return "<STDOUT>", sys.stdout
    elif output_file is None:
        output_prefix = utility.compose_output_prefix(input_filepath=constraints_file, default="delineate")

        if suffix is None:
            suffix = ""
        outpath = "{}{}.{}".format(output_prefix, suffix,  extension)
        return outpath, open(outpath, "w+")


    else:
        output_prefix = os.path.splitext(os.path.basename(os.path.expanduser(os.path.expandvars(constraints_file))))[0]
        output_prefix= os.path.join(output_file, output_prefix)
        if suffix is None:
            suffix = ""
        outpath = "{}{}.{}".format(output_prefix, suffix,  extension)
        return outpath, open(outpath, "w+")
...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        msg.append("")
        msg.append("    partitions")
        msg.append("    rates")
        msg.append("")
        msg.append("Or type '--help' for help:")
        msg.append("")
        msg.append("    --help")
        msg.append("    partitions --help")
        msg.append("    rates --help")
        msg.append("")
        msg = "\n".join(msg)
        sys.exit(msg)
    ...
    if hasattr(args, 'func'):
        print(args.constraints_file)
        print(args.tree_file)
        print(args.output_prefix)
        args.func(args.tree_file, args.constraints_file,
        args.output_prefix, args.extra_info_field_value,
        figtree_display_label='species-lineage',
        no_primary_tree_file=args.no_primary_tree_file,
        no_translate_tree_tokens=args.no_translate_tree_tokens,
        preserve_underscores=args.preserve_underscores,
        report_constrained_cumulative_probability_threshold=args.report_constrained_cumulative_probability_threshold,
        report_constrained_probability_threshold=args.report_constrained_probability_threshold,
        report_mle_only=args.report_mle_only,
        speciation_completion_rate=args.speciation_completion_rate,
        speciation_completion_rate_estimation_initial=args.speciation_completion_rate_estimation_initial,
        speciation_completion_rate_estimation_max=args.speciation_completion_rate_estimation_max,
        speciation_completion_rate_estimation_min=args.speciation_completion_rate_estimation_min,
        store_relabeled_trees=args.store_relabeled_trees,
        tree_format=args.tree_format,
        tree_info=args.tree_info,
        truncated_tree_file_size=args.truncated_tree_file_size,
        underflow_protection=args.underflow_protection,
        describe= False,
        version= False)
    else:
        pass
